...
def predict_and_get_result(model, data):
...
if st.button("결과 확인하기"):
    ...
    if all_questions_answered == False:
        ...
        data = pd.read_csv(r'reponse1.csv')

        y1 = data['이과 1 문과 0'].values
        x1 = []

        for i, rows in data.iterrows():
            x1.append([rows["매우 그러함 5, 조금그러함 4, 보통임3, 조금 아님2, 매우 아님1"], rows['과학적 원리를 이해하는 것이 재미있나요?'],
                       rows['답이 여러개인 문제를 한개인 문제보다 선호하시나요?'], rows['비문학보다 소설을 읽는 것을 더 좋아하시나요?'],
                       rows['definition 1 justice 0'], rows['본인이 논리적이라고 생각하시나요?'], rows['평소 수학 문제 푸는 것을 즐기시나요?'],
                       rows['5팩토리얼 1, 오! 0'], rows['둘다 잇음4, 통과3, 통사2, 둘다 업음1'], rows['264 1, 시인0']])
        ...
        for i, rows in data.iterrows():
            x2.append([rows["계획을 하고 행동하는 편인가요?"], rows['방 정리하는 것을 즐기는 편인가요?'], rows['갑자기 생긴 약속을 즐기는 편인가요?'],
                       rows['다이어리를 꾸준히 쓰는 편인가요?'], rows['평소 창의적이라는 말을 자주 듣나요?'], rows['일이 계획대로 되지 않으면 큰 불안감을 느끼나요?'],
                       rows['공부할 때 본인이 좋아하는 부분부터 하는 경향이 있나요?'], rows['미래를 통제하고 싶어하나요?'], rows['사물함 정리가 잘 되어있는 편인가요?'],
                       rows['항상 일정한 시간에 일어나고 잠드는 편인가요?']])
        ...
        with st.expander('**WBTI 결과**'):
            ...
            with tab1:
                final_score1_2d = np.array([final_score1])
                final_score1 = predict_and_get_result(model1, final_score1_2d)
                if np.around(final_score1, 1)[0] != 1:  # 수정된 부분
                    result = 'L'
                else:
                    result = 'N'

                final_score2_2d = np.array([final_score2])
                final_score2 = predict_and_get_result(model2, final_score2_2d)
                if np.around(final_score2, 1)[0] == 1:  # 수정된 부분
                    result += 'J'
                else:
                    result += 'P'
            ...
            with tab2:
                ...
                if np.around(final_score1, 1)[0] != 1:
                    st.success("##### \u2606 국어")
                    st.success(a1_1)
                    st.text("")
                    st.success("##### \u2606 수학")
                ...
                if np.around(final_score2, 1)[0] == 1:
                    st.success(b111)
                else:
                    st.success(b222)